...
docker volume create n8n-quick 2>/dev/null || true

echo "🔧 Starting N8N container (Custom CE with SSO)..."
docker run -d \
  --name $CONTAINER_NAME \
  -p $PORT:5678 \
  -e N8N_PORT=5678 \
...
# docker run -d --name n8n-quick -p 5678:5678 -e N8N_HOST=0.0.0.0 -e N8N_PORT=5678 n8nio/n8n:latest
